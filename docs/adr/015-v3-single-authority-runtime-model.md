...
A work-available notification may shorten discovery latency, but it never grants execution ownership. PostgreSQL polling/claim remains the correctness path.
...
BrokerNative is at-least-once. External side effects must tolerate duplicate execution. KubeJob does not currently provide a BrokerNative Inbox/deduplication store, so `IdempotencyKey` must not imply duplicate suppression in this mode.
